...
class ControlSurfacePidController(controller_interface.BaseController):
    ...
    supported_input_types = ['pitch', 'roll', 'pos_', 'for_pos_z', 'for_vel_x']
    ...
    def control(self, data, controlled_state):
        ...
        control_command, detail = self.controller_wrapper(
                ...
                control_param={'P': self.settings['P'],
                               ...
                               'D': self.settings['D']},
                i_current=i_current)
        if self.settings['input_type'] != 'for_vel_x':
            controlled_state['aero'].control_surface_deflection += (
                np.array(self.settings['controlled_surfaces_coeff'])*control_command)
        else:
            controlled_state['structural'].steady_applied_forces[0][1] += -control_command 
            # controlled_state['structural'].steady_applied_forces[0][1] += controlled_state['structural'].steady_applied_forces[1][1] + data.thrust -control_command 
    ...
    def extract_time_history(self, controlled_state):
        ...
        if self.settings['input_type'] == 'pitch':
            ...
            output = euler[0]
        elif 'pos_(' in self.settings['input_type']:
            node_str = self.settings['input_type']
            node_str = node_str.replace('pos_(', '')
            ...
            output = pos[2]
        elif self.settings['input_type'] == 'for_pos_z':
            step = controlled_state['structural']
            output =step.for_pos[2]

        elif self.settings['input_type'] == 'for_vel_x':
            step = controlled_state['structural']
            output =step.for_vel[0]
        else:
            raise NotImplementedError(
