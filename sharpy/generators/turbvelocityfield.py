...
D
    ...
    def read_xdmf(self, in_file):
        ...
        # Zack: Ensure this is correct in xdmf
        ...
        self.grid_data['time'] = np.fromstring(tree.Domain.Time.DataItem.text,
                                               sep=' ',
                                               count=2,
                                               dtype=float)
        ...
        # OLD CODE THAT MAKES NO SENSE WITHOUT CONTEXT
    ...
    def generate(self, params, uext):
        ...
        self.interpolate_zeta(zeta,
                              for_pos,
                              uext, t) # ZKK remove t when done debugging
    ...
    def get_field_bbox(self, x_grid, y_grid, z_grid, frame='G'):
        ...
        # ZKK getting rid of wierd transformations
        # if frame == 'G':
    ...
    def interpolate_zeta(self, zeta, for_pos, u_ext, t, interpolator=None, offset=np.zeros((3))):
                ...
                for i_n in range(n_n):
                    ...
                    for i_dim in range(3):
                        try:
                            ...
                            if i_dim == 2 and i_m == 0 and isurf == 0:
                                z_coord.append(coord)
                                z_test.append(u_ext[isurf][i_dim, i_m, i_n]) 
                                zeta_hold.append(zeta[isurf][:, i_m, i_n])
                        except ValueError:
                            print(coord)
                            raise ValueError()
    ...
    def read_grid(self, i_grid, i_cache=0):
        ...
        for i_dim in range(3):
            ...
            if i_cache == 0:
                if not self.settings['store_field']:
                    ...
                    with h5py.File( self.route + '/' + file_name, 'r') as file:
                        dataset = file['data'][()]
                        data = dataset[:]

                    self.vel_holder0[i_dim] = np.memmap(self.route + '/' + 'mem_' + file_name,
                                                        dtype=self.grid_data['grid'][i_grid][velocities[i_dim]]['Precision'],
                                                        shape=(self.grid_data['dimensions'][0],
                                                        self.grid_data['dimensions'][1],
                                                        self.grid_data['dimensions'][2]),
                                                        order='C',
                                                        mode = 'w+')
                    self.vel_holder0[i_dim][:] = data
                    # Discard the original variable containing the loaded dat
                    del data
                    ...
                    self.vel_holder0[i_dim] = (np.fromfile(open(self.route + '/' + file_name, 'rb'),
                                                          dtype=self.grid_data['grid'][i_grid][velocities[i_dim]]['Precision']).\
                                                          reshape((self.grid_data['dimensions'][0],
                                                                   self.grid_data['dimensions'][1],
                                                                   self.grid_data['dimensions'][2]),
                                                                   order='C'))
                ...
                if not self.settings['store_field']:
                    ...
                    self.vel_holder1[i_dim] = np.memmap(self.route + '/' + 'mem_' + file_name,
                                                        dtype=self.grid_data['grid'][i_grid][velocities[i_dim]]['Precision'],
                                                        shape=(self.grid_data['dimensions'][0],
                                                        self.grid_data['dimensions'][1],
                                                        self.grid_data['dimensions'][2]),
        ...
        # file_name = self.grid_data['grid'][i_grid]['U']['file']
        # for ii in range(3):
        #     if i_cache == 0:
        #         if not self.settings['store_field']:
        #             # load file, but dont copy it
        #             self.vel_holder0 = np.memmap(self.route + '/' + file_name,
        #                                             # dtype='float64',
        #                                             dtype=self.grid_data['grid'][i_grid]['U']['Precision'],
        #                                             shape=(self.grid_data['dimensions'][2],
        #                                                     self.grid_data['dimensions'][1],
        #                                                     self.grid_data['dimensions'][0]),
        #                                             order='F')
        #         else:
        #             # load and store file
        #             self.vel_holder0 = (np.fromfile(open(self.route + '/' + file_name, 'rb'),
        #                                                     dtype=self.grid_data['grid'][i_grid]['U']['Precision']).\
        #                                                     reshape((self.grid_data['dimensions'][2],
        #                                                             self.grid_data['dimensions'][1],
        #                                                             self.grid_data['dimensions'][0]),
        ...
        #         interpolator.append(self.create_interpolator(self.vel_holder0,
        #                                                 self.grid_data['initial_x_grid'],
        #                                                 self.grid_data['initial_y_grid'],
        #                                                 self.grid_data['initial_z_grid'],
        #                                                 i_dim= 0))
        ...
        #         if not self.settings['store_field']:
        #             # load file, but dont copy it
        #             self.vel_holder1[:] = np.memmap(self.route + '/' + file_name,
        #                                             # dtype='float64',
        #                                             dtype=self.grid_data['grid'][i_grid]['U']['Precision'],
        #                                             shape=(self.grid_data['dimensions'][2],
        #                                                     self.grid_data['dimensions'][1],
        ...
        #         else:
        #             # load and store file
        #             self.vel_holder1 = (np.fromfile(open(self.route + '/' + file_name, 'rb'),
        #                                                     dtype=self.grid_data['grid'][i_grid]['U']['Precision']).\
        #                                                     reshape((self.grid_data['dimensions'][2],
        #                                                             self.grid_data['dimensions'][1],
        #                                                             self.grid_data['dimensions'][0]),
        ...
        #         interpolator.append(self.create_interpolator(self.vel_holder1,
        #                                                 self.grid_data['initial_x_grid'],
        #                                                 self.grid_data['initial_y_grid'],
        #                                                 self.grid_data['initial_z_grid'],
        #                                                 i_dim= ii))
