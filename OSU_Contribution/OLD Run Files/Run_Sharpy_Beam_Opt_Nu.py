...
# the air is calm.
u_inf = 15.5 #14 for XHALE, 10 for T_Tail
rho = 1.225 # 1.225 

# default elevator is -0.020708598 radians 3
alpha = 6.0484*np.pi/180 # 1G 2.0251 alpha, .0533 N thrust, 3.7940 degrees elevator / 2G 4.287 degrees alpha, .2173 N thrust, 6.9285 degrees elevator 
beta = 0
roll = 0
gravity = 'on'
global g_loading
...
thrust = 10.21 # 1G .0533 N  / 2G .2173 N  
...
global min_thick
global bb
global h, e, g, max_deflection 

global guess_x_loc
guess_x_loc = np.linspace(0, 1, 6)

max_strain = .01
max_deflection = 4 # 4 meters in z deflection
beam_save_file = '2G_Beam_Opt_6_Points_2nd_Go.xlsx'
min_thick = .00003
bb =  0.2999980281284993 
h = 0.11706065387250503
...
global diff_sum_out, guess_log, wing_tip_deflection_log, max_deflection_val, wing_deflection_diff_log
guess_log = []
diff_sum_out = []
masss = []
...
low = (e * i_yy_min + ei_other)
high = (e * ((bb*h**3)/ 12) + ei_other)
...
def local_for_forces(data, tstep=None):
    ...
    for i in range(len(applied_forces_copy)):
        if i == 0:
            together[i, ...] = applied_forces_copy[i]*.5 + gravity_forces_copy[i]
        else:
...
def beam_opt(guess):
    ...
    guess_final = list(np.array(guess)*10000)
    ...
    #     guess_final.append(guess[i]*mag[i])
    
    # # calculating stiffnesses based on 
    # def poly_calc(span, guess):
    #     test_re = np.zeros_like(span)
    #     for i in range(len(span)):
    #         result = 0
    #         for j in range(len(guess)):
    #             result += guess[j]*span[i]**j
    #             if j == len(guess)-1:
    #                 test_re[i] = result
    ...
    for i in range(len(beam_opt_nodes)):
        ...
        ei_span.append(f(fem['coordinates'][i][1]/max_span))
    ...
    for i in ei_span:
        ...
        Eqn = sp.Eq((bb*h**3)/ 12 - ((bb-2*t)*(h-2*t)**3)/12, i_yy[-1])
        ans = sp.solve(Eqn)
        thickness.append(ans[0])
        area_spar.append((bb*h) - (bb - 2*thickness[-1]) * (h - 2*thickness[-1]))
        gj_spar =  (4 * (bb*h) ** 2) / ((2*bb + 2*h)/ (g*thickness[-1])) # GJ from the spar only
    ...
    for i in range(len(thickness)):
        if i != len(thickness) -1 and thickness[i] < thickness[i+1]: # zero indicates a solid piece do not need to change this 
            ...
            for i in range(i + 1):
                thickness[i] = hold
                area_spar[i] = bb*h - (bb - 2*thickness[i]) * (h - 2*thickness[i])
                i_yy[i] = (bb*h**3)/ 12 - ((bb-2*thickness[i])*(h-2*thickness[i])**3)/12
    ...
    diff_strain = masss_temp**2 # wing_deflection_diff**4 
    
    # penalties
    
    # if divergence
    if data.structure.div_flag.value == 1:
        diff_strain = diff_strain**5 + diff_strain 
    
    else:
        # if max strain is exceeded
        if max(strain) > max_strain:
            diff_strain = diff_strain**3 + diff_strain 

        # if max wing tip deflection is exceeded
        if max_deflection_val < data.structure.timestep_info[-1].pos[beam_opt_nodes[-1]][2]:
            diff_strain = diff_strain**3 + diff_strain 
        
        
        # if wing_deflection_diff >= 0:
        #     diff_strain = wing_deflection_diff**2 + diff_strain
    ...
    diff_sum_out.append(diff_strain)
    wing_tip_deflection_log.append(data.structure.timestep_info[-1].pos[beam_opt_nodes][-1][2])
    
     
    df = pd.DataFrame([guess_log, diff_sum_out, masss, wing_deflection_diff_log, wing_tip_deflection_log, max_deflection_val_log]) #, thickness, area_spar, node_forces, ei_span, gj_span, shear, moment, strain])
    df.to_excel(beam_save_file, header= False, sheet_name= 'Optimizer Log')
    
    # defining log results
    ...
    if data.structure.div_flag.value != 1:
        ...
        node_forces_z_log= np.array(node_forces[0:len(beam_opt_nodes), 2])
        grav_forces_log = np.array(grav_forces[0, ])
...
#     span = []
#     for i in range(len(beam_opt_nodes)):
#         span.append(fem['coordinates'][i][1])
...
#             current = data.structure.timestep_info[-1].pos[i]
#             moment[i] = shear[i+1] * (back[1] - current[1]) + moment[i+1]
#             ei_node[i] = (moment[i] * h * .5) / max_strain
#             if ei_node[i] < low:
#                 ei_node[i] = low
...
#     # finding nodes that need to be curve fit
#     ei_fit = []
#     span_fit = []
#     for i in range(len(ei_node)):
#         if ei_node[i] == high:
#             continue
...
#         ei_fit.append(ei_node[i])
#         span_fit.append(span[i])

#     ei_fit = np.array(ei_fit)
#     span_fit = np.array(span_fit)
#     if ei_fit.size == 0:
#         ei_fit = ei_node
#         span_fit = span
#     hold = np.polyfit(span_fit, ei_fit, 2)
#     # reverse ordering of np array
#     hold = hold [::-1]

#     return hold 

# test = np.linspace(50000, 15000, 10)
# for i in test: 
# beam_opt([1.94052758, 1.94052758, 1.94052758, 0.91312668])
...
#         number = number // 10
#         count = count + 1 
#     mag.append(10**(count-1))
#     guess_final.append(guess[i]/mag[i])

ei0 = [6.96958558, 6.96958555, 6.96958551, 6.39283312, 6.39283302, 6.39283267]
bnds = []
for i in range(len(ei0)):
     bnds.append([low/10000, high/10000])

cons = []
for i in range(len(ei0)-1):
    cons.append({'type': 'ineq', 'fun': ei_con(i).non_increase})
    
results = minimize(beam_opt, ei0, constraints= cons,  method = 'SLSQP', bounds = bnds)
print(results)
# beam_opt(results)


# results_final = minimize(beam_opt, results.x, constraints= cons,  method = 'SLSQP', bounds = bnds)
# beam_opt([8.73730308, 8.7373031,  8.73730311, 7.5595763,  7.55957623])
