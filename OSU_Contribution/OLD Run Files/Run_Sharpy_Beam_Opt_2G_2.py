...
cs_deflection = -2.5*np.pi/180 # initial control surface deflection 1G .06621779 / 2G 0.1209251 
...
beam_save_file = 'New_Updated_Cost_Result.xlsx'
min_thick = .0001
max_thick = .003
bb =  .11999999999
h =  .13235760
...
global diff_sum_out, guess_log, wing_tip_deflection_log, max_deflection_val, wing_deflection_diff_log, max_strain_log
guess_log = []
diff_sum_out = []
masss = []
max_strain_log = []
...
global guess_x_loc, mag, starting_mass
num_points = 6
guess_x_loc = np.linspace(0, 1, num_points)
ei0 = [5.32]*num_points
mag = 1e5
starting_mass = 106
...
def beam_opt(guess):
    ...
    guess_final = list(np.array(guess)*mag)
    ...
    #         for j in range(i + 1):
    #             thickness[j] = hold
    #             area_spar[j] = bb*h - (bb - 2*thickness[j]) * (h - 2*thickness[j])
    #             i_yy[j] = (bb*h**3)/ 12 - ((bb-2*thickness[j])*(h-2*thickness[j])**3)/12
    #             ei_span[j] = ei_other + e * i_yy[j]
    ...
    masss_val = abs(grav/(9.81*g_loading)) 
    ...
    for i in range(len(strain)):      
        diff_strain += (max(0, strain[i] - max_strain) / max_strain)**2  
      
    diff_strain += len(strain)*max(0, (deflection-max_deflection_val)/max_deflection_val)**2 
    ...
    masss.append(masss_val) 
    diff_sum_out.append(masss_val/starting_mass + diff_strain )
    wing_tip_deflection_log.append(data.structure.timestep_info[-1].pos[beam_opt_nodes][-1][2])
    
    df = pd.DataFrame([guess_log, diff_sum_out, masss, max_strain_log, wing_tip_deflection_log, max_deflection_val_log])
    ...
    return masss_val/starting_mass + diff_strain 
...
# beam_opt([1.16123563, 1.16123563, 1.16123563, 1.16123563, 1.16123563,
#        1.16123563])

results = minimize(beam_opt, ei0, bounds = bnds) #, constraints= cons)
print(results)
