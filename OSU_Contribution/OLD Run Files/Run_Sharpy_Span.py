...
beam_save_file = 'Flex_Span_Opt.xlsx'
min_thick = .0001
max_thick = .003
low_twist = -10
high_twist = 10
...
def span_opt(guess):
    ...
    # if strain is over max strain
    for i in range(len(strain)):      
        penalties += (max(0, strain[i] - max_strain) / max_strain)**2  
    
    # if max deflection constraint is violated
    ...
    # if divergence in 2g trim
    if data.structure.div_flag.value == 1:
        max_strain_log.append(0)
        masss_temp = starting_mass*5
        power = starting_power*5
        penalties = 0 
    ...
    ei_log.append(data.structure.stiffness_db[0][4][4])
    gj_log.append(data.structure.stiffness_db[0][3][3])
    mass_log.append(data.structure.mass_db[0][0][0])
    
     
    df = pd.DataFrame([guess_log, power_log, diff_sum_out, penalties_log, masss, chord_log, alpha_log, elevator_log, wing_tip_deflection_log, max_deflection_val_log, ei_log, gj_log, mass_log])
