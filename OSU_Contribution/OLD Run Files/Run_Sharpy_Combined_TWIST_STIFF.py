...
beam_save_file = 'Both_Stiff_Twist.xlsx'
min_thick = .0001
max_thick = .003
low_twist = -10
high_twist = 10
...
global num_ei, num_twist
num_ei = 5
num_twist = 3
x_loc_ei = np.linspace(0, 1, num_ei)
x_loc_twist = np.linspace(0, 1, num_twist + 1)
...
for i in range(len(x_loc_twist)-1):
     bnds.append([low_twist, high_twist])


# combined guess of ei and twist
guess = ei
for twist in twist:
    guess.append(twist)
...
def beam_opt(guess):
    ...
    ei_guess = []
    for i in range(len(x_loc_ei)):
        ei_guess.append(ei[i]*mag)
    
    twist = []
    for i in range(len(x_loc_ei), len(guess)):
        if i == len(x_loc_ei):
    ...
    cost = (power / starting_power + masss_temp / starting_mass) 
