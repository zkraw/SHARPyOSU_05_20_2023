...
beam_save_file = 'Default_Twist.xlsx'
...
global thickness, thickness_x_loc 
thickness = [0.0014741,  0.00081279, 0.0006251,  0.0005,     0.0005,     0.0005]
thickness_x_loc = np.linspace(0, 1, 6)  

global starting_power
# guess = [0.13263946, 0.2378833, 1.07808361, 2.47055216]
guess = [0.26936273, 0.2942012,  0.86067023, 2.67949387]
# guess = [0, 0, 0, 0]
guess_x_loc = np.linspace(0, 1, 5)
max_twist = 10
min_twist = -10
starting_power = 375
...
bnds = []
for i in range(len(guess_x_loc)-1):
    bnds.append(np.array([min_twist, max_twist]))
...
def inv_forces(data, tstep=None):
    if tstep is None:
        tstep = data.structure.timestep_info[data.ts]
    applied_forces = data.structure.nodal_b_for_2_a_for(data.NU_forces,
                                                                tstep)
...
def span_opt(guess):
    ...
    f = interp1d(np.array(thickness_x_loc), np.array(thickness), kind= 'linear')
    
    twist = []
    for i in range(len(guess)):
        if i == 0:
            twist.append(0*np.pi / 180)
        twist.append(guess[i]*np.pi/ 180)
    
    twist_f = interp1d(guess_x_loc, twist, kind = 'linear')
    ...
    for i in range(len(beam_opt_nodes)):
        ...
        thickness_all.append(f(fem['coordinates'][i][1]/max_span))
    ...
    for i in lines:
        ...
        if current[0] == 'initial_alpha': 
            current[1] = str(3.766778435*np.pi/180) + '\n'
            nu_lines.append(' = '.join(current))
            continue

        if current[0] == 'initial_deflection':
            current[1] = str(-1.326530467*np.pi/180) + '\n'
            nu_lines.append(' = '.join(current))
            continue
    ...
    tstep = data2.structure.timestep_info[-1]
    f, m = inv_forces(data2)
    
    # f_xnu = sum(data2.NU_forces[:, 0])
    # f_ynu = sum(data2.NU_forces[:, 1])
    # f_znu = sum(data2.NU_forces[:, 2])
    ...
    if data2.structure.div_flag.value != 1:
        ...
        power = 410 # handled by increase in mass 
    ...
    df = pd.DataFrame([guess_log, masss, return_log, penalties_log, ms_tu_log, ms_su_log, ms_cu_log, chord_log, alpha_log, elevator_log, wing_tip_deflection_log, max_deflection_val_log]) #, ei_log, gj_log, mass_log])
