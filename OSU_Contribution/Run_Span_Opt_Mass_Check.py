...
#  start at nonlinear static to get mass
set['flow'] = ['BeamLoader',
        'AerogridLoader',
        # 'NonLinearStatic',
        #'StaticUvlm',
...
set['alpha'] = 6*np.pi/180
set['cs_deflection'] = -5*np.pi/180
set['initial_thrust'] = 88.6765895305575

spar_cap_dict['core'] = .005

# Output File
beam_save_file = 'AR_66_CL_6_MS_1.xlsx'

# Span Mass Spar Creation
global span, surface_area, guess_x_loc, max_deflection, mag, control_points, min_ms, desired_cl
surface_area = 32
ar = 66
span = (ar*surface_area)**.5
min_ms = 1 
desired_cl = .6
...
from OSU_Contribution.utils.Dyn_Min_Spar_Func.init_sharpy_settings import t_tail_static
t_tail_static(route, case_name, set)
...
#---------------------------------------------------------------------------------------------------------------------
#-------------------------STATIC COUPLED MASS CHECK------------------------------------------------------------------- 
#---------------------------------------------------------------------------------------------------------------------
    
    # static coupled just to check what the payload mass should be 
    # from OSU_Contribution.utils.Dyn_Min_Spar_Func.sharpy_run_settings import nonlinear_static
    # g_load = nonlinear_static(route, case_name, logs, set)
    
    # # running mass check using sharpy
    # mass_check = sharpy_main.main(['', route + '/' + case_name + '.sharpy'])

    # # calculating the mass of the configuration
    # grav_forces = mass_check.structure.timestep_info[0].gravity_forces
    # grav = 0
    # for i in range(len(grav_forces)):
    #     grav += np.sqrt(grav_forces[i][0]**2 + grav_forces[i][1]**2 + grav_forces[i][2]**2)
    
    # mass_check_val = abs(grav/(g_load*9.81))
    # mass_desired = (set['rho']*set['u_inf']**2*surface_area*desired_cl) / (2*9.81)
    # payload_add = mass_desired - mass_check_val
    
    # # only one lumped mass for T-Tail need to specify if using other configurations
    # fem['lumped_mass'][0][0] = fem['lumped_mass'][0][0] + payload_add 
    # logs['payload'].append(fem['lumped_mass'][0][0])
...
#---------------------------------------------------------------------------------------------------------------------
    ...
    # setting set to stratic trim 
    # set['flow'] = ['BeamLoader', 'AerogridLoader','StaticTrim', 'BeamLoads', 'AerogridPlot', 'BeamPlot']
    ...
    from OSU_Contribution.utils.Dyn_Min_Spar_Func.sharpy_run_settings import bad_g_trim
    g_load = bad_g_trim(route, case_name, logs, set)
...
# #---------------------------------------------------------------------------------------------------------------------   
    ...
    # masss_temp = abs(grav/(set['g_loading']*9.81)) 
