...
# sys.path is a list of absolute path strings
direct = __file__.split('/')
del direct[-1]; del direct[-1]; del direct[-1]; del direct[-1]
direct =  '/'.join(direct)
sys.path.append(direct)
...
def generate_aero_file():
    ...
    control_surface_deflection[0] =  .4 # cs_deflection
...
case_data = sharpy_main.main(['', route + '/' + case_name + '.sharpy'])
...
def plot_sim(data):
    ...
    # ax.set_xlabel('X [m]')
    # ax.set_ylabel('Z [m]');
    plt.savefig('Velocities_Plots.png')
