...
# sys.path.append('/mnt/c/Users/Zack/Desktop/python/sharpy/')
...
# ------------------------------------------------------------------------------------------------------
    ...
    def excel(self, data):
        ...
        # Loading DAT FILES
        dat = pd.read_excel(data, sheet_name = 'DAT Files')
        rows = dat.shape[0]
        columns = dat.shape[1]
        dats = []
        for i in range(0, columns, 2):
            ...
            for j in range(1, rows):
                if str(dat.values[j, i]) != 'nan' and str(dat.values[j, i+1]) != 'nan':
                    temp.append([dat.values[j, i], dat.values[j, i+1]])
                    if j == rows -1:
                        dats.append(temp)
                else:
                    dats.append(temp)
            ...
            for j in range(1,3):
                ...
                # calculating new airfoil sections from interpolating and preventing duplicate airfoil production
                    if len(value_history) == 0:
                        ...
                        # writing dat file for XFOIL Calculation
                        file_name = os.getcwd() + 'Temp_Interp.dat'
                        check_file = os.getcwd() + '/OSU_Contribution/XFOIL6.99/check_interp/check.txt'
                        
                        aero_dat = self.airfoil_inter(airfoil_temp[i,j], dats)

                        if os.path.isfile(file_name):
                            os.remove(file_name)
                            
                        if os.path.isfile(check_file) == False:
                            interp_write_flag = True
                            # writing check file 
                            with open(check_file, 'w') as check:
                                for iii in range (len(aero_dat)):
                                    check.write(str(aero_dat[iii][0]) + '     '  + str(aero_dat[iii][1]) + '\n')
                        
                        #writing dat file so it can be checked with reference file
                        with open(file_name, 'w') as f: 
                                for iii in range(len(aero_dat)):
                                    f.write(str(aero_dat[iii][0]) + '     '  + str(aero_dat[iii][1]) + '\n')
                        
                        #interp airfoil data
                        if self.interp_check(check_file, file_name) == False:
                            interp_write_flag = True
                            interp_polars.append(self.xfoil_oper(chord[i, j]))
                            # writing check file 
                            with open(check_file, 'w') as check:
                                for iii in range (len(aero_dat)):
                                    check.write(str(aero_dat[iii][0]) + '     '  + str(aero_dat[iii][1]) + '\n')
...
# -----------------------------------------------------------------------------------------------------
        ...
        # interpolation polar inclusion
        if interp_write_flag == True:
            for i in range(len(interp_polars)):
                polar.append(np.array(interp_polars[i]))
            
            rb = xlrd.open_workbook(data)
             # copy the contents of excel file
            wb = copy(rb)
            # open the first sheet
            w_sheet = wb.get_sheet('Polars')
            column = 0
            for i in range(len(polar)):
                ...
                for j in range(len(polar[i])):
                    row += 1
                    for k in range(4):
                        if str(polar[i][j][k]) == 'nan':
                            break
                        w_sheet.write(row, column + k, polar[i][j][k])
            wb.save(data)
            # polar = np.array(polar, dtype= float)
