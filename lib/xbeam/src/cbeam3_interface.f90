...
contains
    subroutine cbeam3_solv_nlnstatic_python(n_elem,&
        ...
        integer(c_int), intent(INOUT)      :: div_flag
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!-> Subroutine CBEAM3_ASBLY_STATIC_PYTHON
!
!-> Description:
!
!    Assembly system matrices for the static case
!
!-> Remarks.-
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 subroutine cbeam3_asbly_static_python(num_dof,&
                                        n_node,&
                                        n_elem,&
                                        pos_ini,&
                                        psi_ini,&
                                        ...
                                        fdof,&
                                        options,&
                                        nodal_gravity_forces,&
                                        Kglobal,&
                                        Qglobal,&
                                        iLoadStep)bind(C)
 ! subroutine cbeam3_asbly_static_python(n_node,&
 !                                        n_elem,&
 !                                        ! dt,&
 !                                        pos_ini,&
 !                                        psi_ini,&
 ...
 !                                        iLoadStep)bind(C)
  ...
  !
    ...
    integer(c_int), intent(IN)      :: n_node
    integer(c_int), intent(IN)      :: n_elem
    integer(c_int), intent(IN)      :: iLoadStep
  !
    ! input variables: deformations
    real(c_double), intent(IN)      :: pos_ini(n_node, 3)
    real(c_double), intent(IN)      :: psi_ini(n_elem, max_elem_node, 3)
  ...
  !
    ...
    real(c_double), intent(OUT)     :: Kglobal(num_dof,num_dof)
    real(c_double), intent(OUT)     :: Qglobal(num_dof)
    real(c_double), intent(OUT)     :: nodal_gravity_forces(n_node, 6)
  ...
  !   real(c_double)                  :: MSS_gravity(num_dof+6, num_dof+6)
  !   real(c_double)                  :: MRS_gravity(6, num_dof+6)
    real(c_double)                  :: MRR(6, 6)
    real(c_double)                  :: gravity_forces(6)
  !
    ! variable initialization
    type(xbelem)                    :: elements(n_elem)
    type(xbnode)                    :: nodes(n_node)
  ...
  ! &         reshape((/1.d0,0.d0,0.d0,0.d0,0.d0,1.d0,0.d0,0.d0,0.d0,0.d0,1.d0,0.d0,0.d0,0.d0,0.d0,1.d0/),(/4,4/))
    ...
    ! DeltaX = 0.d0
    Fglobal = 0.0d0
    call cbeam3_asbly_static (num_dof, n_elem, n_node,elements,nodes,pos_ini,psi_ini,&
                            pos,psi,&
                            steady_applied_forces*dble(iLoadStep)/dble(Options%NumLoadSteps), &
                            Kglobal,Fglobal,Qglobal,Options,Mglobal,MRR)

    ! Include applied forces in the vector
    Qglobal= Qglobal - dble(iLoadStep)/dble(Options%NumLoadSteps) * &
    &              MATMUL(Fglobal,fem_m2v(steady_applied_forces,num_dof,Filter=ListIN))

    ! Include gravity forces
    if (options%gravity_on) then
    nodal_gravity_forces = -fem_v2m(MATMUL(Mglobal,&
                                     cbeam3_asbly_gravity_static(num_dof + 6,&
                                                                 options)),&
                                     n_node, 6)

    ! ams: I think gravity_forces are not used
    gravity_forces = -MATMUL(MRR, cbeam3_asbly_gravity_static(6, options))

    Qglobal = Qglobal - dble(iLoadStep)/dble(Options%NumLoadSteps)*&
              fem_m2v(nodal_gravity_forces, num_dof, filter=ListIN)
    end if
...
end subroutine cbeam3_asbly_static_python
