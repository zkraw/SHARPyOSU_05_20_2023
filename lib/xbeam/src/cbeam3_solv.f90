...
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 subroutine cbeam3_solv_nlnstatic_old (NumDof,n_elem,div_flag,n_node,Elem,Node,AppForces,nodal_gravity_forces,Coords,Psi0, &
...
! I/O Variables.
  ...
  integer,      intent(OUT)  :: div_flag ! checks for divergence so that optimziation code can keep running 
...
! Local variables.
  ...
  real(8)                           :: old_dx
    ...
    do iter = 1, options%maxiterations + 1
        ...
        if (Iter > 1) then
            if (residual/old_DX < options%MinDelta) then
                converged = .TRUE.
